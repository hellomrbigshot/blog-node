...
router.get('/weibo/callback', (req, res, next) => {
  ...
  fetch(path, {
    ...
    .then(weibo_res => {
      ...
      fetch(
        `${WEIBO_USER_API}?access_token=${weibo_res.access_token}&uid=${weibo_res.uid}`
      )
        .then(info => info.json())
        .then(weibo_info => {
          oauthAction(
  ...
  if (session_user) {
    ...
    UserModel.getUserByOauthInfo({ type, name }).then(user => {
      if (user) {
        ...
        // 如果没有注册，就跳转到注册界面
        res.redirect(
          `${REGISTER_URL}?name=${name}&type=${type}&domain=${domain}&avatar_url=${avatar_url}`
        )
      }
    })
  }
